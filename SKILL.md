...
本技能将 OpenAI Codex 作为第二位 AI 审查员，对 Claude 产出的代码变更进行交叉审查。核心理念：不同模型有不同的盲区，Codex 能看到 Claude 看不到的问题。
...
| Codex 登录 | 已认证 | `codex whoami` |
| Codex CC 插件 | 已安装 | 检查 `/codex:review` 是否可用 |
...
1. **有未提交更改** → `git diff` 输出（包括 unstaged + staged）
2. **无未提交更改，有最近 commit** → `git diff HEAD~1..HEAD`
...
**同步模式**：
```bash
codex exec --model gpt-5.5 --effort <level> "<prompt>"
```

同时传入当前的 git diff 作为上下文：
```bash
git diff | codex exec --model gpt-5.5 --effort <level> "请审查以下代码变更：..."
```

**后台模式**：
```bash
codex exec --model gpt-5.5 --effort <level> --background "<prompt>"
```
...
但在官方插件不支持自定义 prompt 注入的情况下，优先使用 `codex exec` CLI 直接调用。

### 步骤 6：解析结果并更新记忆

从 Codex 的审查输出中：

1. **提取 Claude 犯的错误**：仅提取 🔴 阻断项 和 🟡 重要问题
2. **对每个错误分类**：
   - `logic-error`：逻辑错误
   ...
   - `performance`：性能问题
3. **更新记忆文件**：
   - 新错误类型 → 添加到 mistakes 数组，frequency = 1
   - 已有错误类型 → frequency += 1，更新 lastSeen
   - 合并相似错误（编辑距离判断）
4. **同时写入两层**：
   - AI 痕迹类错误 → 同时写入全局和项目级
   - 项目特定错误 → 仅写入项目级
...
新增错误时，检查是否与已有记忆相似：
- 同一分类 + 编辑距离 < 30% → 合并到已有记忆（frequency += 1, 更新 lastSeen 和 examples）
- 否则 → 新建记忆条目
